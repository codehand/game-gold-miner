...
describe('upgrade control view model', () => {
  ...
  it('affords a control at exactly its price, and not one unit below', () => {
    ...
    expect(
      createUpgradeControlViewModel(target, cost, cost).isAffordable,
  ...
  it('marks controls affordable only from the current balance', () => {
    ...
    expect(poor.floors[0].upgradeControl?.isAffordable).toBe(false);
    expect(poor.elevator.upgradeControl.isAffordable).toBe(false);
    expect(poor.warehouse.upgradeControl.isAffordable).toBe(false);
    expect(rich.floors[0].upgradeControl?.isAffordable).toBe(true);
    expect(rich.elevator.upgradeControl.isAffordable).toBe(true);
    expect(rich.warehouse.upgradeControl.isAffordable).toBe(true);
  });
  ...
  it('gives every control a distinct key and its own command target', () => {
    ...
    keys.forEach((key, index) => {
      expect(key).toBe(upgradeTargetKey(controls[index].target));
    });
    expect(upgradeTargetKey({ type: 'mine-shaft', floorId: 'floor-1' })).not.toBe(
      upgradeTargetKey({ type: 'mine-shaft', floorId: 'floor-2' }),
    );
  });
});

describe('upgrade press feedback', () => {
  it('labels each outcome and marks only a purchase positive', () => {
    const at = (outcome: 'purchased' | 'insufficient-funds' | 'unavailable') => {
      return describeUpgradeFeedback(createUpgradeFeedback(outcome, 1_000), 1_000);
    };
  ...
  it('shows a result until it expires, then clears itself', () => {
    const feedback = createUpgradeFeedback('purchased', 1_000);
    const lastVisibleMs = 1_000 + UPGRADE_FEEDBACK_DURATION_MS - 1;

    expect(describeUpgradeFeedback(feedback, 1_000)).not.toBeNull();
    expect(describeUpgradeFeedback(feedback, lastVisibleMs)).not.toBeNull();
    expect(
      describeUpgradeFeedback(feedback, lastVisibleMs + 1),
      'the result must not outlive its duration',
    ).toBeNull();
    expect(describeUpgradeFeedback(null, 1_000)).toBeNull();
  });
  ...
  it('clears a result the clock has moved behind', () => {
    ...
    expect(describeUpgradeFeedback(feedback, 999)).toBeNull();
    expect(describeUpgradeFeedback(feedback, Number.NaN)).toBeNull();
  });

  it('rejects a result stamped with an unusable clock reading', () => {
    expect(() => createUpgradeFeedback('purchased', Number.NaN)).toThrow(
      /finite presentation timestamp/,
    );
  });
});

describe('upgrade commands through the driver', () => {
  it('buys a mine shaft once, deducting exactly the displayed price', () => {
    ...
    expect(driver.purchaseUpgrade({ type: 'mine-shaft', floorId: 'floor-1' })).toBe(
      'purchased',
    );
  ...
  it('buys the shared elevator and warehouse through their own commands', () => {
    ...
    expect(driver.purchaseUpgrade({ type: 'elevator' })).toBe('purchased');
    expect(driver.purchaseUpgrade({ type: 'warehouse' })).toBe('purchased');
  ...
  it('refuses an unaffordable purchase without changing anything', () => {
    ...
    expect(driver.purchaseUpgrade({ type: 'elevator' })).toBe(
      'insufficient-funds',
    );
    expect(driver.purchaseUpgrade({ type: 'warehouse' })).toBe(
      'insufficient-funds',
    );
  ...
  it('refuses a shaft on a locked floor', () => {
    ...
    expect(driver.purchaseUpgrade({ type: 'mine-shaft', floorId: 'floor-4' })).toBe(
      'unavailable',
    );
  ...
  it('charges against the gold the mine has now, not the last rendered frame', () => {
    ...
    expect(driver.snapshot.floors[0].upgradeControl?.isAffordable).toBe(false);
