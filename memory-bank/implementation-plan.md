...
**Instructions:** Show separate English upgrade controls for each mine shaft, the shared elevator, and the shared warehouse. Every open-stage Level badge opens a detail popup without buying; show authoritative current/next attributes and bottom CTAs for x1, x5, and MAX affordable sequential levels. Enable each purchase only when affordable, send the matching authoritative atomic batch command, and provide immediate feedback.

**Test:** Use E2E flows to prove opening any stage popup changes no state, unavailable batches are disabled, x1 deducts one exact cost, MAX buys the exact affordable quantity, attributes and choices update in place, and overlay input cannot activate gameplay underneath. Cover mine-shaft, elevator, and warehouse batch upgrades.
