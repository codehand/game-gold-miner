...
The completed establishment phase now provides the web scaffold, quality tooling, enforced module boundaries, and a minimal 360×640 Phaser boot scene. The deterministic core runs the automatic production chain, exposes a bottleneck-aware effective rate, and supports exact-price upgrades, cumulative level 10/25/50/100 milestone multipliers, and sequentially gated floor unlocks. A deterministic ten-minute balance harness confirms the provisional curve opens all four floors, reaches milestones without runaway level-100 growth, and does not stall. The complete authoritative snapshot and effective rate cross a strict version-1 save boundary and persist as one replaceable local IndexedDB record. Malformed or incompatible saves recover automatically to a playable fresh game. Valid returning players see a simple modal for a deterministic pending reward based on the saved rate, at 50% efficiency for at most two hours; claiming adds the exact amount once and persists it before the modal closes. The portrait mine screen now has its responsive frame: a fixed English top HUD, a shared surface strip for the elevator and warehouse, and a scrollable mine area that runs to the bottom edge with no navigation bar reserved for deferred features. The canvas fits inside the device safe area at phone, tablet, and desktop sizes without cropping any control. That frame is now filled: each of the four floors shows its number, mine-shaft level, a placeholder miner, a growing material pile, an extraction progress bar, and a shaft-upgrade control, while locked floors appear plainly locked with no miner and no control. The shared elevator and warehouse show their own level, capacity, held material, and cycle progress beside their upgrade controls, so the whole production chain is legible at a glance from a saved game. The mine now runs while the player watches: each of the three stages advances its own indicator from the live simulation, and material waiting anywhere in the chain is drawn as a stack that turns red and says `Backed up` once a whole cycle of work is queued, so the slowest stage is visible without reading a number. Miners keep swinging and conveyors keep moving as decoration only — that motion is on a separate clock and cannot change how much gold the mine produces. The top bar is now live too: it shows the gold the player can actually spend and an estimate of what the mine earns per second, both shortened so they stay readable as the numbers grow — thousands as `1.2K`, and past trillions as `14.6aa`. The income figure is the rate the whole chain can deliver, not what the shafts alone could dig, so widening the stage everything is waiting on visibly raises it. The mine can now be improved: every open floor and both shared stages carry a button showing what the next level costs, lit up only while the player can afford it. Pressing one buys that level immediately — the gold falls, the level rises, the button shows the new higher price, and the income figure moves if the stage that was holding the chain up just got wider. A press the player cannot pay for says `Need more gold` on the button itself rather than failing silently, and a purchase is saved without waiting for the player to leave. The mine can now also be deepened: every locked floor states the floor and level it is waiting on and what opening it costs, and its button lights up only once both are true — gold alone will not open a floor whose shaft requirement is unmet, and pressing it early says `Level too low` rather than nothing. Buying it opens the floor where it stands: the panel loses its locked look, a miner appears and starts working, and the unlock button becomes that floor's own upgrade button, with no reload or restart. The floor is still open, and still producing, after the player closes the game and comes back. The mine is now reachable with one thumb: dragging or wheeling anywhere over it scrolls the shafts past a top bar and surface strip that stay put, so the deepest floor's button can be reached and pressed, and every button is big enough to hit. A drag that moves the mine no longer buys whatever it happens to end on — a swipe scrolls, a tap buys, and a tap that wobbles slightly still buys. Placeholder art remains deferred to its ordered step.
