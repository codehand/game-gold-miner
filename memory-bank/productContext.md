...
The completed establishment phase now provides the web scaffold, quality tooling, enforced module boundaries, and a minimal 360×640 Phaser boot scene. The deterministic core runs all unlocked floors concurrently through the final automatic mine → shared elevator → warehouse → spendable-gold order and exposes a bottleneck-aware effective production estimate. The player-facing runtime and rate display, upgrade commands, and later progression behaviors have not been introduced yet.
