...
The completed establishment phase now provides the web scaffold, quality tooling, enforced module boundaries, and a minimal 360×640 Phaser boot scene. The deterministic-core phase now has provisional validated balance data, a large-number boundary, renderer-free authoritative state, bounded fixed-step timing, and automatic mine-floor extraction into local queues. The player-facing runtime and the elevator, warehouse, spendable-gold, and progression behaviors have not been introduced yet.
