...
The completed establishment phase now provides the web scaffold, quality tooling, enforced module boundaries, and a minimal 360×640 Phaser boot scene. The deterministic core now completes the automatic mine → shared elevator → warehouse → spendable-gold chain with capacity-limited timed stages and conserved material. The player-facing runtime, final all-stage ordering, production-rate display, and progression behaviors have not been introduced yet.
