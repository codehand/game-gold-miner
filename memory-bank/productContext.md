...
The completed establishment phase now provides the web scaffold, quality tooling, enforced module boundaries, and a minimal 360×640 Phaser boot scene. The deterministic core now runs all unlocked floors concurrently through the final automatic mine → shared elevator → warehouse → spendable-gold order, with same-tick handoffs, capacity-limited timed stages, inert locked floors, and conserved material. The player-facing runtime, production-rate display, and progression behaviors have not been introduced yet.
