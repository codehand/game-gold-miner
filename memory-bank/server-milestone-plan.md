...
> **Step 18 interaction (recorded 2026-09-13).** A pre-milestone save is a
> version-1 save document, and Step 18 bumped the current schema to version 2
> (adding `warehouse.totalOfflineGoldClaimed`). `loadActiveGame` migrates a
> version-1 save to version 2 on load, defaulting the counter to `"0"`, so the
> document that gets adopted and later returned is the migrated version-2
> document, not the original version-1 bytes. The migration is lossless — every
...
implemented and is the current validation gate. Step 30 is deferred with its
reason recorded in `progress.md`; Step 31 is implemented and awaiting
validation. Step 32 must not begin until the Step 31 validation gate is passed.**
...
of it, because the two had drifted. Per-step evidence, packages, and review
findings are in `archive/step-implementation-map.md`; the prose account of how
each phase unfolded is in `archive/phase-narrative.md`.
