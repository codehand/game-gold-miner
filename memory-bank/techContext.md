...
Steps 1 through 24 are complete. Step 25 is implemented with passing automated checks and is awaiting user validation; Step 26 has not started. The browser calculates and settles capped saved-rate offline income, presents positive rewards in a simple modal, force-persists one exact claim before dismissal, and renders a safe-area-aware 360×640 portrait layout with a fixed HUD, a shared surface strip, and a camera-clipped scrollable mine area. No relational/server database or physics system exists.
...
- `BootScene` publishes `data-layout-viewport`, `data-layout-hud`, `data-layout-surface`, `data-layout-mine`, `data-layout-mine-content-height`, and `data-layout-bottom-navigation` on the canvas alongside the existing boot/renderer diagnostics.
