...
Steps 1 through 25 are complete. Step 26 is implemented with passing automated checks and is awaiting user validation; Step 27 has not started. The browser calculates and settles capped saved-rate offline income, presents positive rewards in a simple modal, force-persists one exact claim before dismissal, and renders a safe-area-aware 360×640 portrait layout whose scrollable mine now shows four floor views and whose surface strip shows the shared elevator and warehouse, all bound to the loaded core snapshot. No relational/server database or physics system exists.
...
- `src/main.ts` now composes balance validation, IndexedDB loading/recovery, Phaser startup into the dedicated `#game-viewport` parent, lifecycle saves, pending reward presentation, and claim persistence. The Step 24 modal remains the only DOM overlay.
