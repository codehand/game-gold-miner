...
Steps 1 through 27 are complete. Step 28 is implemented with passing automated checks and is awaiting user validation; Step 29 has not started. The browser calculates and settles capped saved-rate offline income, presents positive rewards in a simple modal, force-persists one exact claim before dismissal, and renders a safe-area-aware 360×640 portrait layout whose scrollable mine shows four floor views and whose surface strip shows the shared elevator and warehouse. The mine is now live: a simulation driver advances the core to the injected wall clock on every rendered frame, all three production stages show their own progress indicator and queued material, and a separate cosmetic animation clock drives decoration only. The top HUD reports authoritative gold and the bottleneck-aware income estimate through one shared abbreviated formatter. No relational/server database or physics system exists.
...
- `BootScene` publishes `data-hud-view`, `data-floor-views`, `data-surface-views`, and `data-animation` at most every 100 ms rather than every frame, because displayed progress changes continuously and an unthrottled read-back would serialize the whole screen 60 times a second for diagnostics alone.
- `src/game/layout/` is pure Phaser-free geometry and palette data, so `tests/unit/layout.test.ts` and the Playwright layout spec both import the `src/game/layout` barrel without loading Phaser. `eslint.config.mjs` enforces that purity for `src/game/layout/**` (no `document`/`window`/`navigator`, no `phaser` import) and `tests/unit/architecture.test.ts` probes the rule. It exports the 360×640 constants, `calculateMineLayout`, `calculateMineContentHeight`, `calculateFloorSlotRegion`, and `serializeRegion`, and rejects non-finite/non-positive dimensions, heights below 412 logical pixels, invalid floor counts, and negative floor indexes.
- The portrait layout tiles `hud` (`0,0,360,72`), `surface` (`0,72,360,140`), and `mine` (`0,212,360,428`) with no gaps and no reserved bottom navigation. Four floor slots produce 514 logical pixels of mine content, so the mine area must scroll.
...
- Phaser 4 removed WebGL geometry masks (`setMask` logs a warning and does nothing), so the mine area is clipped by a dedicated camera viewport instead. The main camera ignores the mine content layer, the mine camera ignores the fixed HUD/surface layers, and Step 31 will scroll only that camera.
...
- `BootScene` publishes `data-layout-viewport`, `data-layout-hud`, `data-layout-surface`, `data-layout-mine`, `data-layout-mine-content-height`, `data-layout-bottom-navigation`, `data-hud-view`, `data-floor-views`, and `data-surface-views` on the canvas alongside the existing boot/renderer diagnostics.
...
- `MineFloorView` and `SharedStageView` in `src/game/entities/` build their game objects once inside a supplied layout region, rebind only through `applySnapshot`, and expose `describeRenderedState` so browser tests read values back from the rendered objects. Locked floors switch to `LOCKED_PANEL_BACKGROUND` with a `Locked` badge, muted text, no placeholder miner, and no upgrade control. Upgrade controls are presentational until Step 29.
- `createGame(parent, viewModel)` constructs `BootScene` with the loaded snapshot, so the mine boots already bound instead of showing placeholder values first. Surface stage panels start at y 28 inside the surface strip and are 102 logical pixels tall; the region geometry published in Step 25 is unchanged.
...
- `npm run test`: one hundred fifty-nine tests across the scaffold, architecture, balance-configuration, large-number, authoritative-state, simulation pipeline, production-rate, upgrade, milestone, unlock, economy-progression, save-schema, persistence, save-recovery, offline-income, pending-reward claim, duration-display, portrait-layout, palette, and mine view-model coverage pass.
- `npm run test:e2e`: seven Chromium tests pass: the fresh boot/reload smoke test sees no offline modal, a controlled returning-player flow claims and persists the exact reward without recreating it after reload, and four viewport tests (narrow phone 320×568, tall phone 390×844, tablet portrait 768×1024, desktop 1280×800) assert the canvas fits inside the safe box, the HUD/surface/mine regions stay on screen in the documented order, the mine reaches the bottom edge, no bottom navigation exists, no console errors occur, and four pixel probes prove both cameras cross-ignore correctly. The layout spec forces `preserveDrawingBuffer` through `page.addInitScript` because Phaser disables it for performance; production rendering is unchanged. A seventh test seeds a known version-1 save whose four floors differ in lock state, level, progress, and queued material, then compares every rendered floor number, level, lock state, and progress value — plus both shared stages — against that snapshot, with eight further pixel probes covering the unlocked and locked panels, both progress fills and tracks, and the material pile.
