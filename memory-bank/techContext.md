...
Steps 1 through 11 are complete. Step 12 is implemented with passing automated checks and is awaiting user validation; Step 13 has not started. The pure core now exports immutable large numbers, authoritative state contracts, deterministic fresh-state construction, bounded fixed-step advancement, mine-floor extraction, shared-elevator transport, and warehouse conversion. No physics system or persistence schema exists yet.
...
- Each unlocked floor advances extraction from the base balance's cycle duration. Completed cycles add `baseYield × outputGrowthRate^(level - 1)` to local material and total-extracted `GameNumber` values, preserve normalized overflow progress, and do not alter spendable gold or downstream stage state. Milestone multipliers remain deferred to Step 17.
...
- Step 11 advances elevator state before extraction inside each fixed tick, so new extraction becomes pickup-eligible on the next tick. The final extraction → elevator → warehouse update order remains deferred to Step 13.
...
- Step 12 advances warehouse conversion before elevator and extraction inside each fixed tick, so new elevator deliveries become conversion-eligible on the next tick. Final full-pipeline ordering remains deferred to Step 13.
...
- `npm run test`: thirty-eight tests across the scaffold, architecture, balance-configuration, large-number, authoritative-state, fixed-step timing, extraction, elevator, and warehouse suites pass.
