...
| 24 — Rejection handling | Implemented on 2026-09-14, awaiting user validation; a 2026-09-14 review found and fixed five issues (two HIGH, one MEDIUM, two LOW), re-verified | `save-sync`'s `handleSaveUpload` writes exactly one append-only `save_audit` row per authenticated `PUT /v1/save` attempt through the new `writeSaveAudit` dep (`writeSaveAuditViaServiceRole`; `save_audit` grants no client role any access, so this is the function's second and last service-role use). The row carries `outcome` (`accepted`/`rejected`), `error_code` (present exactly when rejected), the client's claimed `base_revision`, the server's `resulting_revision` (present exactly when accepted), `document_bytes`, `client_reported_at` (the document's own `savedAtTimestampMs`, recorded and never trusted), and a `detail` with the server-authored reason (a Step 23 bound violation's `{counter, claimed, maximum}`, a validation `reason`, a conflict's `serverRevision`, the size cap, or a malformed-body reason). Accepted attempts are recorded too; the write is best-effort (logged, never fatal); unauthenticated requests write nothing. The player-facing half was already built by Step 19 and is now pinned: a `save_rejected` is terminal-but-keep-syncing, so the replica drops that document (shape-suppressed; a forced trigger retries and a success clears it), keeps the local save and the session playable, and shows the `cloud-sync-save-rejected` notice. Review fixes: **H1 (HIGH)** — the client instant is bounded to the `timestamptz` round-trippable range (years 0001–9999) with the raw out-of-range value kept in `detail.clientReportedAtOutOfRangeMs`; previously a year-10000+ value formatted to an extended-year ISO string Postgres refuses, aborting the insert and erasing the attempt's row; **M1 (MEDIUM)** — an unexpected collaborator failure (`readCurrentSave`/`writeSaveRow` throwing, or the non-`SaveDocumentError` rethrow) is caught and recorded as a `rejected`/`server_error` row before the 500; **H2 (HIGH)** — `baseRevision` is validated against §5 (null or a positive safe integer; anything else is a recorded `400 malformed_request`) before it reaches the audit's `bigint` column, and `writeSaveAuditViaServiceRole` coerces non-safe-integer revisions to null and clamps `document_bytes`; **L1 (LOW)** — the Content-Length path marks its `document_bytes` as `declaredBytes`; **L3 (LOW)** — the two remaining unaudited-500 paths (`request.text()` on an aborted body, and building a `409` from an out-of-band-corrupted stored row) are caught and recorded too; **L2 (LOW)** — the per-rejection service-role round trip is carried into Step 25. Evidence: 8 Deno unit tests; `tests/unit/server-stack.test.ts` pins `admin.from('save_audit').insert(`; `tests/server-integration/save-audit.integration.test.ts` (6 live tests including RLS invisibility/unwritability, an unrepresentable client clock still writing its row, and invalid `baseRevision` values each writing their own row); `tests/server-e2e/save-rejection.spec.ts` (stubbed `422 save_rejected` → local save intact, production continues, one notice, no uncaught error). All gates pass: `verify` (unit 637, E2E 52, build, production smoke), `verify:server` (server unit 112, integration 91, server-e2e 9). |
| 25 — Abuse limits | Implemented 2026-09-17; validated and merged | Shared per-user/per-address fixed-window limiter and body-size guard; mutation-proven adversarial coverage. |
| 26 — Adversarial suite | Implemented 2026-09-17; validated and merged | Nine attacks, including the migration-derived direct-write matrix, each mutation-proven. |
| 27 — Leaderboard storage | Implemented 2026-09-18; validated and merged | Lifetime-gold metric, all-time board, and updated-at tie-break pinned against the real table. |
| 28 — Leaderboard writes | Implemented 2026-09-18; awaiting validation | Accepted-save-only service-role publication from `save-sync`, with real integration coverage. |
| 29 — Leaderboard display | Implemented 2026-09-19; awaiting validation | Public read function, authenticated own-rank projection, Rewards modal, and offline retry state. |
| 30 — Decide on friends | Deferred 2026-09-19 | The verified all-time board supplies the current asynchronous social goal; discovery/privacy/moderation requirements are not defined. |
| 31 — Entitlements | Implemented 2026-09-19; awaiting validation | Server-checked cosmetic entitlement; no payment flow or client grant path. |
| 32 — Audit log | Implemented 2026-09-19; awaiting validation | `account_audit` migration, boundary triggers, post-mint redemption audit, service-only RPC, seven-event integration coverage, and derived seven-table RLS matrix. Review fix: cascaded identity removal now records a null account link so deletion cannot fail its FK; regression covered. |
| 33–37 | Not started | Blocked by the Step 32 validation gate. |
