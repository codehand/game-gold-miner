...
The 2026-09-08 review leaves fifteen authoritative floors in progressive groups of five and a 52-pixel HUD whose centre is `warehouse.inputQueue`. The surface tower, pour, and every loaded cart now empty exactly when that queue is zero; the elevator cannot descend past a floor whose queue remains; and every surface hauler/cargo cart shares one route baseline. Tapping a mine-floor Level badge opens a non-purchasing detail popup with mining attributes and x1/x5/MAX upgrade choices, while the shared-stage badges remain direct upgrades.
