...
Steps 1 through 26 are complete. Step 27's live production-stage visualisation — a pull-based simulation driver, per-stage progress indicators and queued-material displays, and a cosmetic animation clock that cannot influence output — is implemented with passing automated checks and is awaiting user validation. Step 28 and all later work remain blocked. IndexedDB schema version 1 is unchanged and documented below; there is no relational or server database.
...
| `src/game/view-model/mineViewModel.ts`, `src/game/view-model/stageAnimation.ts`, `src/game/view-model/index.ts` | Pure Phaser-free presentation logic. The snapshot view model derives per-floor heading, level, lock status, progress ratio and label, queued-amount label, discrete pile height and backlog state, plus each shared stage's level, capacity, held amount, queue blocks, running/backed-up status, and cycle progress. The animation module holds the cosmetic clock maths — frame accumulation with a speed multiplier, miner swing, conveyor travel, and the progress-driven cycle marker. Provisional amount formatting lives here until Step 28. |
| `src/game/runtime/MineSimulationDriver.ts`, `src/game/runtime/index.ts` | Phaser-free live bridge between the core and the screen: holds authoritative state, advances it to an injected wall clock on each pull, memoizes the derived snapshot, and accepts state replaced by a command. |
| `src/game/entities/MineFloorView.ts`, `src/game/entities/SharedStageView.ts`, `src/game/entities/index.ts` | Reusable Phaser views that build their own game objects once, rebind through `applySnapshot`, move decoration through `applyAnimation`, and report what they actually display through `describeRenderedState`. |
...
| `src/game/scenes/BootScene.ts` | Single scene that builds the fixed HUD layer, the shared surface layer with both stage views, and the mine content layer with four floor views; pulls the newest snapshot from its source on every frame, advances the separate cosmetic animation clock from the frame delta, clips the mine through a dedicated camera viewport, and records startup, renderer, layout, rendered-view, and animation diagnostics on the game canvas. |
