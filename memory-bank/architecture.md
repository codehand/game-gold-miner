...
Steps 1 through 31 are complete. Step 32 and its approved Step 32A layout/animation revision are implemented with passing automated checks and await user validation. The first production-oriented pack adds semantic floor art, miner/unloader loops, and a moving elevator cabin with cargo cat without changing simulation or input contracts. Step 33 and all later work remain blocked. IndexedDB schema version 1 is unchanged and documented below; there is no relational or server database.
...
| `src/game/assets/placeholderAssets.ts` | Semantic Phaser texture keys and public paths for the original Step 32 family plus the Step 32A floor/elevator pack loaded by `BootScene.preload`. |
...
The layout rejects non-finite or non-positive dimensions and any height below `HUD_HEIGHT + SURFACE_HEIGHT + MINE_MIN_HEIGHT` (436). `calculateMineContentHeight()` returns 572 logical pixels for four 288×132 floor slots — taller than the 404-pixel mine region — so the area scrolls by 168. `calculateFloorSlotRegion(index)` returns each slot relative to the content origin. The 48-pixel shaft exposes explicit cabin, rail, and plaque constants and rejects structures that cannot fit.
...
Each production stage has an authoritative indicator. For a floor, extraction progress feeds the ping-pong patrol across the full corridor from the unloader to the gold pile, so position and facing freeze with a paused core while walk frames stay cosmetic. The shared elevator and warehouse retain a cycle bar plus marker. A full floor pile stays in the approved gold artwork and grows to its maximum size without a text overlay; warehouse backlog keeps its dedicated stage cue.
