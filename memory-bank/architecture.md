...
Steps 1 through 28 are complete. Step 29's upgrade controls — a priced, affordability-aware button on every unlocked mine shaft and on both shared stages, each routing a press to its own core command and showing the result — are implemented with passing automated checks and are awaiting user validation. Step 30 and all later work remain blocked. IndexedDB schema version 1 is unchanged and documented below; there is no relational or server database.
...
| `src/game/view-model/mineViewModel.ts`, `src/game/view-model/hudViewModel.ts`, `src/game/view-model/upgradeControl.ts`, `src/game/view-model/formatAmount.ts`, `src/game/view-model/stageAnimation.ts`, `src/game/view-model/index.ts` | Pure Phaser-free presentation logic. The snapshot view model derives per-floor heading, level, lock status, progress ratio and label, queued-amount label, discrete pile height and backlog state, plus each shared stage's level, capacity, held amount, queue blocks, running/backed-up status, and cycle progress, and carries the HUD model. The HUD module derives the English gold and income captions and their values from authoritative gold and the core's effective production rate. The upgrade-control module derives each control's action caption, price, affordability, command target, and stable key, plus the lifetime of a press result. The format module is the single abbreviated-amount formatter every displayed quantity goes through. The animation module holds the cosmetic clock maths — frame accumulation with a speed multiplier, miner swing, conveyor travel, and the progress-driven cycle marker. |
...
| `src/game/entities/HudView.ts`, `src/game/entities/MineFloorView.ts`, `src/game/entities/SharedStageView.ts`, `src/game/entities/UpgradeControlView.ts`, `src/game/entities/index.ts` | Reusable Phaser views that build their own game objects once, rebind through `applySnapshot`, move decoration through `applyAnimation`, show press results through `applyUpgradeFeedback`, and report what they actually display through `describeRenderedState`. `HudView` owns the fixed top bar: its background, divider, and four text objects. `UpgradeControlView` is the one pressable button, shared by the floor panels and both shared stages. |
...
`MineFloorView` and `SharedStageView` build their game objects once inside a supplied `LayoutRegion` and change only through `applySnapshot`. A floor shows its number badge, heading, mine-shaft level, placeholder miner, material pile, extraction progress bar with percentage, and a shaft-upgrade control. Locked floors use the `LOCKED_PANEL_BACKGROUND` panel with a `Locked` badge, muted text, no miner, and no upgrade control. The elevator and warehouse render side by side in the surface strip with their own level, capacity, held amount, cycle-progress bar, and upgrade control. Upgrade controls became interactive in Step 29, described under the Upgrade Control Contract below; Step 30 adds unlock controls.
...
## Upgrade Control Contract

Every unlocked mine shaft and both shared stages carry one upgrade control. `createUpgradeControlViewModel(target, cost, gold)` is pure and derives the `Upgrade` action caption, the abbreviated price, an `isAffordable` flag from `gold >= cost`, the `UpgradeTarget` naming the command to send, and a stable `key`. Each price comes from the same core function that charges it — `calculateMineShaftUpgradeCost`, `calculateElevatorUpgradeCost`, `calculateWarehouseUpgradeCost` — so the figure on the button and the figure deducted cannot drift. `createMineViewModel` therefore takes each floor's own balance config plus the spendable balance; a locked floor's `upgradeControl` is `null`, because it has no shaft to upgrade.

`UpgradeControlView` is one reusable entity used by both the floor panels (`stacked`: action over price) and the shared-stage panels (`inline`: action left, price right). Its background rectangle carries the hit area, so the pressable region is exactly the drawn one and hiding the control removes it from input, which is how locked floors have no reachable button. An unaffordable control is drawn in `CONTROL_DISABLED_BACKGROUND` with disabled text, but it still accepts a press: refusing in the view would replace the core's answer with the renderer's guess and would leave the player with no feedback at all.

A press goes to `MineSimulationDriver.purchaseUpgrade(target)`, the scene's `MineCommandSink`. The driver advances to the current time first, because the player spends the gold the mine has now rather than the gold the last rendered frame showed, then dispatches to the matching core command and returns `purchased`, `insufficient-funds`, or `unavailable`. A refusal leaves authoritative state and the memoized snapshot untouched, so the scene skips rebinding by identity; a purchase replaces state, re-derives the snapshot, and invokes the optional `onCommandApplied` hook. `src/main.ts` uses that hook to queue a debounced save, because a purchase changes state without any simulation tick completing and would otherwise be lost on the next reload.

The result is shown on the pressed control for `UPGRADE_FEEDBACK_DURATION_MS` (1,200 ms): `Upgraded!` on green, `Need more gold` or `Unavailable` on red, in place of the action and price. `describeUpgradeFeedback(feedback, nowMs)` is pure and expires the message, and the scene runs it on the Phaser scene clock rather than the cosmetic animation clock, so how long a message stays readable cannot change with animation speed. `BootScene` rebinds the snapshot and republishes diagnostics immediately on a press instead of waiting for the next tick. Because that expiry runs on the frame clock, every control re-renders on every frame, so the render path must cost nothing when nothing changed: text colours are compared before they are written, since `Text.setColor` — unlike `Text.setText` — repaints the caption's canvas and re-uploads its texture on every call. A browser test counts repaints across idle frames and requires zero.

`BootScene` publishes `data-upgrade-controls`: every visible control's key, rendered labels, affordability appearance, live feedback, and its pressable rectangle in screen coordinates. Floor controls are drawn through the mine camera, so their world rectangle is offset by that camera's viewport and scroll, while the surface stages are drawn by the main camera where world and screen coincide. This is what lets a browser test aim a real press at a real control instead of assuming a coordinate.
